...
MinigridMLPPotentialNet = models.MinigridMLPPotentialNet
...
def learn(actor_model,
    ...
    with lock:
        ...
        with torch.no_grad():
            potential_next, unused_state = potential_network(encoded_states[1:].detach(), initial_agent_state)
            potential, unused_state = potential_network(encoded_states[:-1].detach(), initial_agent_state)

        intrinsic_rewards = (flags.discounting * potential_next - potential).sum(dim=2)
        intrinsic_rewards *= (count_rewards == 1).float() # Combine episodic state counts with RPF
...
def train(flags):  
    if flags.xpid is None:
        flags.xpid = flags.env + '-rpf-%s' % time.strftime('%Y%m%d-%H%M%S')
    ...
    if 'MiniGrid' in flags.env: 
        ...
        potential_network = MinigridMLPPotentialNet().to(device=flags.device) 
    ...
    def batch_and_learn(i, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            stats = learn(model, learner_model, potential_network,
    ...
    def checkpoint(frames):
        ...
        torch.save({
            ...
            'potential_network_state_dict': potential_network.state_dict(),
